...
pub mod messages_receiver {
    ...
    pub fn log_message_udt_loss(sequence_number: u32, label: &str)  {
        let dt: NaiveTime = Local::now().time();
        println!("[{} - UDT] - SeqNum: {} - {} - Loss", dt, sequence_number, label)
    }

    pub fn log_message_udt_delay(sequence_number: u32, label: &str, delay: &u64)  {
        let dt: NaiveTime = Local::now().time();
        println!("[{} - UDT] - SeqNum: {} - {} - Delay packet for {}ms...", dt, sequence_number, label, delay)
    }
}
